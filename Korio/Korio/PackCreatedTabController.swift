...
class PackCreatedTabController: UIViewController , UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        handle = FIREBASE().ref.child(USER().id).observe(.childAdded, with: { (snapshot) in
            if(snapshot.exists()){
                print("package added!")
                ...
                if(status == "posted" || status == "accepted" || status == "changed" || status == "authorized"){
                    FIREBASE().ref.child(USER().id).child(snapshot.key).child("new").setValue(false)
        ...
        FIREBASE().ref.child(USER().id).observe(.childChanged, with: { (snapshot) in
            if(snapshot.exists()){
                print("package changed!")
                let pack = snapshot.value as? NSDictionary
                let status = pack?.value(forKey: "status") as! String
                if(status != "posted" && status != "accepted" && status != "changed" && status == "reposted" && status != "authorized"){
                    if(self.packages.index(forKey: snapshot.key) != nil){
                        print("package removed!")
                        self.packages.removeValue(forKey: snapshot.key)
                        self.packagesTable?.reloadData()
